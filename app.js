...
const state = {
  ...
  statuses: [],
  jobs: [],
  entries: {},       // {job_id: {YYYY-MM-DD: hours}}
  totalsAll: {},     // kumulativní CELKEM za job napříč týdny
  filterClient: 'ALL',
  filterStatus: 'ALL',
  totalsScope: 'ME', // 'ME' | 'ALL' (Součty: Já/Všichni)
  filterAssignees: [],     // filtr „Grafik“
  newJobAssignees: [],     // assignees pro novou zakázku
}
...
async function loadConfig(){
  try{
    const r = await fetch('./config.json?v=13.1', {cache:'no-store'})
    if(r.ok){
  ...
  throw new Error('Chybí konfigurace Supabase (config.json nebo lokální storage).')
}
async function init(){
...
async function loadClients(){
  const {data, error} = await state.sb.from('client').select('id,name').eq('is_active',true).order('name')
...
async function loadEntriesMine(){
  ...
  const {data, error} = await state.sb
    .from('time_entry')
    .select('job_id,work_date,hours,user_id')
...
async function loadTotalsAll(jobIds){
  ...
  // „Všichni“ – ideálně přes RPC s EXECUTE perms
  const { data:rpcData, error:rpcErr } = await state.sb.rpc('fn_job_totals')
  if(!rpcErr && rpcData){
    const map={}
    for(const r of rpcData){ map[r.job_id]=Number(r.sum_hours||0) }
    return map
  }
  // fallback (pokud RPC není)
  const { data, error } = await state.sb.from('time_entry').select('job_id,hours').in('job_id', jobIds)
  if(error){ showErr(error.message); return {} }
  if(!data?.length){
    showErr('Souhrn za VŠECHNY uživatele je prázdný – pravděpodobně brání RLS. Zvaž RPC fn_job_totals.')
  }
  const map={}
  for(const r of (data||[])){ map[r.job_id]=(map[r.job_id]||0)+Number(r.hours||0) }
  return map
...
function toggleMenu(btn, menu){ const show=menu.hasAttribute('hidden'); document.querySelectorAll('.menu').forEach(m=>m.setAttribute('hidden','')); if(show) menu.removeAttribute('hidden') }
...
function renderTable(){
  ...
  for(const j of visible){
    ...
    const closeB=document.createElement('button'); closeB.className='pill-btn small'; closeB.type='button'; closeB.textContent='Zavřít'
...
try { window.renderTable = renderTable } catch {}

// přepočet řádku + patičky
...
// kliky na bubliny (±0,5h, ne do mínusu)
async function bump(jobId,dateISO,delta){
  try{
    ...
    // přepočet kumulativních „Celkem“
    if(state.totalsScope==='ALL'){
      const { data:rpcData, error:rpcErr } = await state.sb.rpc('fn_job_totals')
      if(!rpcErr && rpcData){
        const row = rpcData.find(r=>r.job_id===jobId)
        if(row){ state.totalsAll[jobId]=Number(row.sum_hours||0); return updateRow(jobId) }
      }
...
// Export do Excelu – skryje řádky bez hodin v daném týdnu, hlavička s uživatelem + rozsahem
...
async function buildShell(){
  ...
  assMenu.onchange = () => {
    ...
    assBtn.textContent = state.filterAssignees.length ? `Grafik: ${state.filterAssignees.join(', ')}` : 'Grafik: Všichni'
    renderTable()
  }
  assClear.onclick = () => {
    state.filterAssignees = []
    assBtn.textContent = 'Grafik: Všichni'
